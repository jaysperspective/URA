...
function MoonDisc({ phaseName }: { phaseName: string }) {
  // Simple "pretty enough" disc; you can swap for a better renderer later.
  return (
    <div className="mx-auto w-[220px] h-[220px] rounded-full bg-white/10 border border-white/10 shadow-inner flex items-center justify-center">
      <div className="w-[206px] h-[206px] rounded-full bg-gradient-to-b from-white/70 to-white/35 opacity-80" />
      <div className="absolute text-white/0">{phaseName}</div>
    </div>
  );
}

function iconFor(kind: Marker["kind"]) {
  // Minimal icons similar to the reference (circle variations)
...
export default function CalendarClient() {
  ...
  const header = useMemo(() => {
    if (!data?.ok) return { top: "CURRENT", mid: "—", sub: "—" };
    return {
      top: "CURRENT",
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur px-6 py-7 text-center">
        ...
        <div className="mt-6 text-white/80 text-xl">
          The Moon is in <span className="text-white font-semibold">{data?.astro.moonSign ?? "—"}</span>
        </div>

        <div className="mt-2 text-white/55 text-sm">
          As of <span className="text-white/70">{data?.gregorian.asOfLocal ?? "—"}</span> • {data?.tz}
        </div>
