...
export default function PivotPrecheckPanel({ initial, onChange }: Props) {
  ...
  const result = useMemo<PivotPrecheckResult>(() => {
    const total =
      s.legBirth + s.structure + s.followThrough + s.timeFit + s.sanity;
    ...
    // - Leg Birth must be strong (>=2)
    // - Structure must be confirmed (==2)
    // - Total must be >= 7
    // - And no auto-fail
    const hardPass =
      !autoFail &&
    ...
    <div style={panel}>
      <div style={headerRow}>
        <div>
          ...
          <div style={subtitle}>
            Score the candidate pivot before you anchor the cycle.
          </div>
        </div>
