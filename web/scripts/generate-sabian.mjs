...
function systemPrompt() {
  return `
You are generating URA's Degree Symbol Reference dataset.

Hard rules:
- Write ORIGINAL text. Do NOT reproduce any canonical degree-symbol phrases.
...
function userPrompt(batch) {
  return `
Generate URA entries for this batch:
...
export const URA_SABIAN: UraSabianEntry[] = ${JSON.stringify(entries, null, 2)};
`;
  fs.writeFileSync(OUT_PATH, header, "utf8");
}

async function main() {
  const apiKey = mustEnv("OPENAI_API_KEY", "URA_OPENAI_API_KEY", "OPENAI_KEY");
